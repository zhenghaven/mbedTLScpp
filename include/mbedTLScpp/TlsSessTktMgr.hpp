...
#endif
{
	...
	template<CipherType _cipherType, size_t _bitSize, CipherMode _cipherMode,
		uint32_t _tktLifetime = MBEDTLS_SSL_DEFAULT_TICKET_LIFETIME>
	class TlsSessTktMgr : public ObjectBase<DefaultTlsSessTktMgrObjTrait>, public TlsSessTktMgrIntf
	{
	public: // Static members:
		...
		static constexpr mbedtls_cipher_type_t sk_mbedCipherType =
				GetMbedTlsCipherType(GetCipherSizedType(_cipherType, _bitSize), _cipherMode);
		static constexpr uint32_t              sk_tktLifetime    =
				_tktLifetime;

	public:

		TlsSessTktMgr(std::unique_ptr<RbgInterface> rand = Internal::make_unique<DefaultRbg>()) :
			_Base::ObjectBase(),
		 ...
		 *
		 */
		virtual void Write(const mbedtls_ssl_session& session, void* start, const void* end, size_t& tlen, uint32_t& lifetime) override
		{
			NullCheck();
			...
			MBEDTLSCPP_MAKE_C_FUNC_CALL(
				...
				Get(), &session,
				static_cast<unsigned char*>(start), static_cast<const unsigned char*>(end),
				&tlen, &lifetime);
		}
